...
void solve()
{
    ...
    while (q--)
    {
        ...
        int position = lower_bound(v.begin(), v.end(), d) - v.begin();
        // cout << position << endl;
        // double dis = d - v[position - 1];
        // double time = (vv[position] - vv[position - 1]) / (v[position] - v[position - 1]);
        // double time_2 = (dis * time);
        // cout << 1LL * (int)vv[position - 1] + (int)time_2 << " ";
        cout << vv[position - 1] + 1LL * (d - v[position - 1]) * (vv[position] - vv[position - 1]) / (v[position] - v[position - 1]) << " ";
    }
